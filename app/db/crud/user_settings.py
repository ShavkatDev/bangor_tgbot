...
async def toggle_daily_digest(telegram_id: int):
    async with async_session_maker() as session:
        ...
        if settings:
            ...
            logging.info(
                f"[Settings] Toggled daily_digest for telegram_id={telegram_id} → {settings.daily_digest}"
            )
...
async def toggle_today_schedule_digest(telegram_id: int):
    async with async_session_maker() as session:
        ...
        if settings:
            ...
            logging.info(
                f"[Settings] Toggled today_schedule_digest for telegram_id={telegram_id} → {settings.today_schedule_digest}"
            )
            await session.commit()

...
async def get_all_users() -> Optional[User]:
    async with async_session_maker() as session:
        ...
        logging.info("[User] Fetching all users")
...
async def get_user_language(telegram_id: int) -> str:
    async with async_session_maker() as session:
        ...
        lang = result.scalar()
        logging.info(
            f"[User] Getting language for telegram_id={telegram_id} → {lang or 'en'}"
        )
        return lang or "en"
...
async def get_attendance_data(telegram_id: int) -> tuple[int, int]:
    async with async_session_maker() as session:
        result = await session.execute(
            select(User.inet_id, User.semester_id).where(
...
async def create_user_with_settings(
    ...
    async with async_session_maker() as session:
        ...
        settings = UserSettings(user_id=new_user.id, language=lang)
        session.add(settings)
        logging.info(
            f"[User] Created user telegram_id={telegram_id}, group_id={group_id}"
        )
...
async def update_user_language(telegram_id: int, lang: str) -> None:
    async with async_session_maker() as session:
        ...
        logging.info(
            f"[User] Updated language to '{lang}' for telegram_id={telegram_id}"
        )
        await session.commit()
...
async def delete_user_completely(telegram_id: int) -> bool:
    async with async_session_maker() as session:
        ...
        if not user:
            logging.warning(
                f"[User] Tried to delete non-existent user telegram_id={telegram_id}"
            )
            return False

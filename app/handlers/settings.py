...
from app.keyboards.reply_keyboard import main_menu_keyboard, settings_keyboard
...
from app.db.crud.user_settings import (
    get_user_settings,
    toggle_daily_digest,
...
async def open_settings(message: types.Message, lang: str):
    ...
    try:
        ...
        logger.error(
            f"Error opening settings menu for user {telegram_id}: {str(e)}",
            exc_info=True,
        )
...
async def open_settings(callback: types.CallbackQuery, lang: str):
    ...
    try:
        ...
        logger.error(
            f"Error returning to settings menu for user {telegram_id}: {str(e)}",
            exc_info=True,
        )
...
async def language_settings(message: types.Message, lang: str):
    ...
    try:
        await message.answer(
            text=LEXICON_MSG["choose_language"][lang], reply_markup=language_keyboard
        )
    except Exception as e:
        logger.error(
            f"Error opening language settings for user {telegram_id}: {str(e)}",
            exc_info=True,
        )
        await message.answer(text=LEXICON_MSG["error"][lang])
...
async def delete_userdata(message: types.Message, lang: str):
    ...
    try:
        ...
        logger.error(
            f"Error showing delete confirmation for user {telegram_id}: {str(e)}",
            exc_info=True,
        )
        await message.answer(text=LEXICON_MSG["error"][lang])
...
async def confirm_delete(callback: types.CallbackQuery, is_admin: bool):
    ...
    try:
        ...
        logger.error(
            f"Error processing delete action for user {telegram_id}: {str(e)}",
            exc_info=True,
        )
        await callback.message.answer(text=LEXICON_MSG["error"][lang])
...
async def process_language_change(callback: types.CallbackQuery, is_admin: bool):
    ...
    try:
        ...
        logger.error(
            f"Error changing language for user {telegram_id}: {str(e)}", exc_info=True
        )
...
async def mailing_settings(message: types.Message, lang: str):
    ...
    try:
        ...
        logger.error(
            f"Error opening mailing settings for user {telegram_id}: {str(e)}",
            exc_info=True,
        )
        await message.answer(text=LEXICON_MSG["error"][lang])
...
async def toggle_daily_digest_handler(callback: types.CallbackQuery, lang: str):
    ...
    try:
        ...
        logger.error(
            f"Error toggling daily digest for user {telegram_id}: {str(e)}",
            exc_info=True,
        )
        await callback.message.answer(text=LEXICON_MSG["error"][lang])
...
async def toggle_today_schedule_digest_handler(
    callback: types.CallbackQuery, lang: str
):
    telegram_id = callback.from_user.id
    username = callback.from_user.username or "No username"
    ...
    try:
        ...
        logger.error(
            f"Error toggling today schedule digest for user {telegram_id}: {str(e)}",
            exc_info=True,
        )
        await callback.message.answer(text=LEXICON_MSG["error"][lang])

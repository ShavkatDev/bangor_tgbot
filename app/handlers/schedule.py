...
async def get_schedule_text(telegram_id: int, lang: str, mode: str) -> str:
    ...
    logger.info(
        f"Getting schedule for user {telegram_id}, mode={mode}, target_day={target_day}"
    )
    ...
    if cache and (datetime.utcnow() - cache.updated_at) < timedelta(hours=9):
        logger.info(
            f"Using cached schedule for group_id={group_id}, updated_at={cache.updated_at}"
        )
...
async def show_today_schedule(message: types.Message, lang: str):
    ...
    try:
        ...
        logger.error(
            f"Error showing today's schedule for user {telegram_id}: {str(e)}",
            exc_info=True,
        )
...
async def show_tomorrow_schedule(message: types.Message, lang: str):
    ...
    try:
        ...
        logger.error(
            f"Error showing tomorrow's schedule for user {telegram_id}: {str(e)}",
            exc_info=True,
        )
        await message.answer(LEXICON_MSG["error"][lang])
...
async def show_week_schedule(message: types.Message, lang: str):
    ...
    try:
        ...
        logger.error(
            f"Error showing week schedule for user {telegram_id}: {str(e)}",
            exc_info=True,
        )
        await message.answer(LEXICON_MSG["error"][lang])
...
async def show_attendance(message: types.Message, lang: str):
    ...
    try:
        ...
        logger.error(
            f"Error showing attendance for user {telegram_id}: {str(e)}", exc_info=True
        )
        await message.answer(LEXICON_MSG["error"][lang])

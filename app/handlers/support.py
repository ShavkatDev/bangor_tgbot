...
from app.db.crud.support import (
    get_open_ticket_by_question_message_id,
    save_ticket,
    close_ticket,
)
...
async def support_command(message: types.Message, lang: str, state: FSMContext):
    ...
    try:
        await message.answer(
            LEXICON_MSG["support_start"][lang], reply_markup=types.ReplyKeyboardRemove()
        )
        await state.set_state(SupportState.waiting_for_question)
    except Exception as e:
        logger.error(
            f"Error starting support conversation for user {telegram_id}: {str(e)}",
            exc_info=True,
        )
...
@support_router.message(
    SupportState.waiting_for_question, F.content_type.in_({"text", "photo"})
)
async def handle_question(message: types.Message, lang: str, state: FSMContext):
    ...
    logger.info(
        f"User {telegram_id} (@{username}) sent {content_type} support question"
    )
    ...
    try:
        ...
        logger.info(
            f"Support ticket created with ID {question_id} for user {telegram_id}"
        )
        ...
        await message.answer(
            LEXICON_MSG["support_sent"][lang], reply_markup=main_menu_keyboard(lang)
        )
    except Exception as e:
        logger.error(
            f"Error processing support question from user {telegram_id}: {str(e)}",
            exc_info=True,
        )
        await message.answer(text=LEXICON_MSG["error"][lang])
...
async def unsupported_type(message: types.Message, lang: str):
    ...
    logger.warning(
        f"User {telegram_id} (@{username}) sent unsupported content type: {content_type}"
    )
    ...
    try:
        ...
        logger.error(
            f"Error handling unsupported content type for user {telegram_id}: {str(e)}",
            exc_info=True,
        )
        await message.answer(text=LEXICON_MSG["error"][lang])
...
async def admin_reply(message: types.Message, lang: str):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Admin {admin_id} (@{admin_username}) replied to question {question_id} with {content_type}"
            )
        ...
        if not ticket:
            logger.warning(
                f"Admin {admin_id} tried to reply to non-existent ticket {question_id}"
            )
            await message.answer(LEXICON_MSG["support_admin_closed"][lang])
            return
        elif ticket.status == "closed":
            logger.warning(
                f"Admin {admin_id} tried to reply to already closed ticket {question_id}"
            )
            await message.answer(LEXICON_MSG["support_admin_closed"][lang])
            return
        ...
        if message.text:
            await message.bot.send_message(
                user_id,
                LEXICON_MSG["support_admin_reply"][lang].format(text=message.text),
            )
        elif message.photo:
            await message.bot.send_photo(
                user_id,
            ...
            logger.warning(
                f"Admin {admin_id} sent unsupported content type: {content_type}"
            )
        ...
        logger.info(
            f"Admin {admin_id} closed support ticket {question_id} for user {user_id}"
        )
        await message.answer(LEXICON_MSG["support_admin_confirm"][lang])

    except Exception as e:
        logger.error(
            f"Error processing admin reply from {admin_id}: {str(e)}", exc_info=True
        )

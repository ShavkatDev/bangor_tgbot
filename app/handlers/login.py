...
async def login_command(message: types.Message, state: FSMContext, lang: str):
    ...
    try:
        ...
        msg = await message.answer(text=LEXICON_MSG["enter_login"][lang])

        await state.update_data(msg=msg, lang=lang)
        await state.set_state(LoginState.waiting_for_login)
    except Exception as e:
        logger.error(
            f"Error in login command for user {telegram_id}: {str(e)}", exc_info=True
        )
...
async def process_login(message: types.Message, state: FSMContext, lang: str):
    ...
    await msg.edit_text(text=LEXICON_MSG["enter_password"][lang])
    await state.set_state(LoginState.waiting_for_password)


@login_router.message(LoginState.waiting_for_password)
async def process_password(
    message: types.Message, state: FSMContext, lang: str, is_admin: bool
):
    telegram_id = message.from_user.id
    username = message.from_user.username or "No username"
    ...
    msg = await message.answer(text=LEXICON_MSG["checking_credentials"][lang])
    ...
    try:
        ...
        if not success:
            ...
            await message.answer(text=LEXICON_MSG["invalid_credentials"][lang])
            await msg.delete()
            await state.clear()
            return
        ...
        if not group_name:
            ...
            await message.answer(text=LEXICON_MSG["group_not_found"][lang])
            await state.clear()
            return
        ...
        try:
            ...
            logger.warning(
                f"Invalid group name format for user {telegram_id} (@{username}): {group_name}"
            )
            group_id = None

        if await is_user_registered(telegram_id):
            logger.info(f"User {telegram_id} (@{username}) already registered")
            await message.answer(text=LEXICON_MSG["already_registered"][lang])
        else:
            ...
            logger.info(
                f"User {telegram_id} (@{username}) successfully registered with group_id={group_id}"
            )
        ...
        logger.error(
            f"Error in password processing for user {telegram_id}: {str(e)}",
            exc_info=True,
        )
        await message.answer(text=LEXICON_MSG["error"][lang])
        await state.clear()
